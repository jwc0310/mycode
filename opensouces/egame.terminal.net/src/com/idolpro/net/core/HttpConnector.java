...
import com.idolpro.net.utils.ByteArrayBuilder;
...
public class HttpConnector {
	...
	return execute(url, opt.mHttpMethod, opt.mSoTimeOut, opt.mConnTimeOut,
		opt.mReconnTimes, opt.mListHeaders, opt.mMapHeaders,
		opt.mPostEntity, opt.mHostKey, cfg.mHosts.get(opt.mHostKey),
		opt.mHttpProxy);
    }
     ...
     * @throws TubeException
     */
    private static EntityResult execute(String url, int method, int soTimeOut,
	    int connTimeOut, int reConnTimes, List<Header> headers,
	    Map<String, String> keyValues, HttpEntity postEntity,
	    String hostKey, LinkedList<String> hosts, HttpHost proxy)
	    throws TubeException {
	...
	if (method == TubeOptions.HTTP_METHOD_GET) {
	    ...
	    req = post;
	    post = null;
	} else if (method == TubeOptions.HTTP_METHOD_PUT) {
	    HttpPut put = new HttpPut(processUrls(url, hostKey, hosts));
	    if (postEntity != null) {
		put.setEntity(postEntity);
	    }
	    req = put;
	    put = null;
	} else if (method == TubeOptions.HTTP_METHOD_DELETE) {
	    HttpDeleteWithBody delete = new HttpDeleteWithBody(processUrls(url,
		    hostKey, hosts));
	    //delete.setEntity(postEntity);
	    /*JSONObject jsonObject=new JSONObject();
	    try {
		jsonObject.put("attentionUserId", "100032");
		jsonObject.put("token", "3ef3c8e0f8be4b2aaa422b9deb8bbaf2");
		jsonObject.put("signature", "452C457AD4498ADF1C9CD290EAD046E4");
	...
	URI newServerURI = URI.create(hosts.get(sIndexMap.get(hostKey)));
    ...
    private static HttpEntity getEntity(HttpClient client, HttpRequestBase req,
	    String hostKey, LinkedList<String> hosts, int reConnTimes)
	    throws TubeException {
	String headerStr = "";
	if (Logger.IS_DEBUG_MODE) {
	...
	for (int i = 0; i <= reConnTimes; i++) {
		...
		if (locHeader != null) {
		    String redirectUrl = locHeader.getValue();
